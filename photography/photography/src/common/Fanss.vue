...
<script>
import { getSelfFollow,getFans,setFollow,removeFollow } from "@/api"
export default {
    props: ["id"],
    filters: {
        setUrl(value){
    ...
    methods: {
        ...
        async follow(){
            let selffollow = await getSelfFollow()
            this.selfFollow = selffollow.message
            let selffans = await getFans(this.id)
            console.log(selffans)
            this.data = selffans.message
        },
        cfollow(index){
            if(!this.selfFollow){
                return false
