<template>
    <div class="remen" @scroll="scrolls">
        <div class="search-box">
            <input type="text" class="search-title" v-model="searchTitle">
        ...
        <div class="works-list" ref="box">
            ...
            >
                <img :src="value.url|url" alt="" @load="a" ref="imgArr" @click="showbox(index)">
...
<script>
import { worksList,userLikes,worksLikes,wRemoveLikes,getCategory,searchWorks } from "@/api"
...
export default {
    ...
    watch: {
		...
		'$route' (to, from) { //监听路由是否变化
			if(to.path == "/discover/works"){ //跳转到哪个页面
                this.getUserLikes()
            }
		},
	},
    ...
    methods: {
            ...
            if(--this.imglength == 0){
                ...
                if(ylength !== glength){
                    ...
                    for(kaishi;kaishi < ylength;kaishi++){
                        // this.$refs.imgArr[kaishi].style.display = "block"
        ...
        async getWorks(){
            let data = await worksList()
            this.list = data.message
            this.imglength = data.message.length
        },
        divshow(index){
        ...
        async liked(_id,index){
            ...
            if(works.classList.contains("icon")){
                let data = await worksLikes(_id)
                if(data.code == 0){
                    this.like = data.message
                }
            }else{
                console.log(index)
                let data = await wRemoveLikes({_id})
                this.like = data.message
            }
        },
        likeds(id){
        ...
        async search(){
            let data = await searchWorks(this.searchTitle,this.category)
