            ...
            <div class="wc-right">
                <div class="wcr-content">
                    ...
                    <div class="edit-box section">
                        <div class="but" @click="editGworks">编辑</div>
                        <div class="but" @click="removeGworks">删除</div>
                    </div>
...
<script>
import { getGWorks,setGComment,deleteGworks } from "@/api"
...
export default {
    ...
    methods: {
        ...
        editGworks(){
            this.$router.push({name:"UpdataGworks",params:{id:this.data._id}})
        },
        async removeGworks(){
            var r=confirm("确定要删除吗？");
            if(r){
                let data = await deleteGworks(this.id)
                if(data.code === 0){
                    this.$router.go(0)
                    console.log("删除成功")
                }
