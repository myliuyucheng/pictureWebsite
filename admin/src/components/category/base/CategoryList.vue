...
<script>
import { getCategoryList,setCategory,addCategory,deleCategory } from "@/api"
export default {
    data(){
        return{
            data: [],
    ...
    methods: {
        ...
        edit(row){
            ...
            this.$prompt("请修改作品分类名称","编辑",{
                ...
                let name = value
                let _id = row._id
                setCategory(_id,name).then(data=>{
                    if(data.code == 0){
                        this.$message({
        ...
        add(){
            this.$prompt("请添加作品分类","添加",{
                confirmButtonText: "添加",
                cancelButtonClass: "取消",
            }).then(({ value })=>{
                addCategory(value).then(data=>{
                    if(data.code == 0){
                        this.$message({
                            message: data.message,
                            type: "success",
