...
<script>
import { getGworksList,getUserList,getCategoryList,deleGworks } from "@/api"

export default {
     methods: {
        async eworks(){
            let data = await getGworksList()
            this.tableData = data.message
        },
        edit(index,value){
            this.$router.push({name:'GworkssBox',params:{_id:value._id}})
        },
        ...
        remove(row){
            const _id = row._id
            this.$confirm("此操作将删除作品集，是否继续？","提示",{
                confirmButtonText: "确定",
                cancelButtonClass: "取消",
                type: "warning"
            }).then(()=>{
                deleGworks(_id).then(data=>{
                    if(data.code == 0){
                        this.$message({
                            type: "success",
